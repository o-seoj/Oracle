...
INSERT INTO ACCOUNT VALUES ('101-13-1005','S3','주택정약예금','941127-1000006', 720800, '2013-10-15 19:56:34');
...
-- 실습 3-6
SELECT * FROM TRANSACTION WHERE TR_TYPE NOT IN(0);
...
-- 실습 3-15
SELECT
    SUM(TR_AMOUNT) 입금총합, AVG(TR_AMOUNT) 입금평균
FROM TRANSACTION WHERE TR_TYPE=1;

-- 실습 3-16
SELECT
    MAX(TR_AMOUNT) "큰 금액", MIN(TR_AMOUNT) "작은 금액"
FROM TRANSACTION WHERE TR_TYPE=2;

-- 실습 3-17
SELECT * FROM ACCOUNT
WHERE ACC_NAME='자유저축예금' AND ACC_BALANCE >= 1000000
ORDER BY ACC_BALANCE DESC;

-- 실습 3-18
SELECT * FROM ACCOUNT WHERE ACC_NAME = '자유저축예금'
ORDER BY ACC_BALANCE DESC OFFSET 0 ROWS FETCH NEXT 1 ROWS ONLY;
...
SELECT * FROM ACCOUNT
WHERE ACC_BALANCE = (SELECT MAX(ACC_BALANCE) FROM ACCOUNT WHERE ACC_TYPE ='S1');
...
-- 실습 3-20
SELECT
    COUNT(CASE TR_TYPE WHEN 0 THEN 1 END) AS "조회 건수",
    COUNT(CASE TR_TYPE WHEN 1 THEN 1 END) AS "입금 건수",
    COUNT(CASE TR_TYPE WHEN 2 THEN 1 END) AS "출금 건수"
FROM TRANSACTION;
...
SELECT
    ...
    ACC_DATE
FROM ACCOUNT A
JOIN CUSTOMER C ON ACC_CID=C_ID;
...
SELECT
    ...
    TR_DATE
FROM TRANSACTION T
JOIN ACCOUNT A ON TR_ACC = ACC_NO
ORDER BY TR_DATE;
